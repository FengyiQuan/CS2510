...
class LightEmAll extends World {
  static final int GAME_ROW = 15;
  static final int GAME_COLUMN = 15;
  ...
  static final int FONT_SIZE = ((GAME_ROW + GAME_COLUMN) / 2 * PIECE_SIZE) / 15;
  ...
  LightEmAll(Random rand, int width, int height) {
    this.rand = rand;
    this.width = width;
    this.height = height;
  ...
  void connectAll() {
    ...
    this.getPower().ratioToPower = 1.0;
    this.getPower().lightThemAll(this.board, this.radius);
  }

  // Part 2: shuffle all game piece
  void shuffleAll() {
    for (ArrayList<GamePiece> col : this.board) {
      u.shuffle(col, this.rand);
    }
  }
  ...
  // generate board by Kruskal's algorithm
  ...
  // Extra Credit: generate board by prim's algorithm
  ArrayList<Edge> generatePrim() {
    ArrayList<Edge> allEdge = this.generateAllEdge();
    ArrayList<Edge> result = new ArrayList<Edge>();
    ArrayList<GamePiece> nodeset = new ArrayList<GamePiece>();
    int vertices = this.width * this.height;
    nodeset.add(this.nodes.get(rand.nextInt(vertices)));

    while (result.size() < vertices - 1) {
      ArrayList<Edge> allConnected = this.getAllConnected(nodeset, allEdge, result);
      allConnected.sort(new SortbyWeight());

      for (int j = 0; j < allConnected.size(); j++) {
        Edge next = allConnected.get(j);

        if (!(nodeset.contains(next.toNode) && nodeset.contains(next.fromNode))) {
          result.add(next);
          nodeset.add(next.fromNode);
          nodeset.add(next.toNode);
          break;
        }
        else {
          continue;
  ...
  // get all connected edge
  ArrayList<Edge> getAllConnected(ArrayList<GamePiece> nodeset, ArrayList<Edge> edge,
      ArrayList<Edge> current) {
    ArrayList<Edge> result = new ArrayList<Edge>();
    for (GamePiece gp : nodeset) {
      for (Edge eg : edge) {
        if ((gp.sameGP(eg.fromNode) || gp.sameGP(eg.toNode)) && !current.contains(eg)) {
          result.add(eg);
        }
      }
    }
      ...
      for (int j = 0; j < this.board.get(i).size(); j++) {
        if (i < this.board.size() - 1) {
          Edge rightLeft = new Edge(this.board.get(i).get(j), this.board.get(i + 1).get(j),
              this.rand.nextInt(15));
          // Extra Credit:change to
          // this.rand.nextInt(2)); to get horizontal bias(smaller weight on horizontal
          // edges)
          // and keep another weight same
        ...
        if (j < this.board.get(i).size() - 1) {
          Edge topDown = new Edge(this.board.get(i).get(j), this.board.get(i).get(j + 1),
              this.rand.nextInt(15));
          // Extra Credit:change to
          // this.rand.nextInt(2)); to get horizontal bias(smaller weight on vertical
          // edges)
          // and keep another weight same
          result.add(topDown);
        }
  ...
  // Part3: modify boards by given edges
  void modifier3() {
  ...
  // Part3: generate all game piece in one array list(nodes)
  void unfoldBoard() {
    for (int i = 0; i < this.board.size(); i++) {
      for (int j = 0; j < this.board.get(i).size(); j++) {
