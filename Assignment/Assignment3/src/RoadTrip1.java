...
class RoadTrip1 {
  ...
  RoadTrip1(String driver1, String driver2, ILoDirection directions) {
...
class MtLoDirection implements ILoDirection {
  ...
  public ILoRoadTripChunk getTripChunk(int miles, int milesSofar, String d1, String d2,
      String currentDriver, ILoDirection directionsSofar) {
    if (directionsSofar.doNeedSwitch()) {
      return new MtLoRoadTripChunk();
    }
    else {
      return new ConsLoRoadTripChunk(new RoadTripChunk(currentDriver, directionsSofar),
          new MtLoRoadTripChunk());
    }
  }

  @Override
  public boolean doNeedSwitch() {
    // TODO Auto-generated method stub
    return false;
  }
...
class ConsLoDirection implements ILoDirection {
  ...
  public ILoRoadTripChunk getTripChunk(int miles, int milesSofar, String d1, String d2,
    ...
    if (this.first.needMoreDirection(milesSofar)) {
      ...
      if (this.rest.doNeedSwitch()) {
        return new ConsLoRoadTripChunk(
            new RoadTripChunk(currentDriver, new ConsLoDirection(
                new Direction("Switch with " + new changeDriver(d1, d2).switchDriver(currentDriver),
                    this.first.getNewMilesSofar(milesSofar)),
                new ConsLoDirection(this.first, directionsSofar)).reverse()),
            this.rest.getTripChunk(miles, miles, d1, d2,
                new changeDriver(d1, d2).switchDriver(currentDriver), new MtLoDirection()));
      }
      else {
        return new ConsLoRoadTripChunk(new RoadTripChunk(currentDriver, new ConsLoDirection(

            this.first, directionsSofar).reverse()),
            this.rest.getTripChunk(miles, miles, d1, d2,
                new changeDriver(d1, d2).switchDriver(currentDriver), new MtLoDirection()));
      }
    }

    else if (milesSofar == 0)
...
class Examples {
  ...
  RoadTrip1 hazelAndHenry = new RoadTrip1("Hazel", "Henry", this.original);
  ...
  boolean testMtLoDirection(Tester t) {
    return t.checkExpect(this.mtr.getTripChunk(10, 3, "abc", "c", "c", this.mtr), // 多出来一个空集最后
        new MtLoRoadTripChunk());
  }
  ...
  boolean testConsLoDirection(Tester t) {
    return t.checkExpect(this.test1.getTripChunk(13, 13, "a", "b", "a", this.mtr), // 多出来一个空集最后
        new ConsLoRoadTripChunk(new RoadTripChunk("a", new ConsLoDirection(this.alber, this.mtr)),
            this.mtc))
        && t.checkExpect(this.test1.getTripChunk(14, 13, "a", "b", "a", this.mtr),
            new ConsLoRoadTripChunk(
                ...
                this.mtc))
        /*
         * && t.checkExpect(this.test1.getTripChunk(10, 10, "a", "b", "a", this.mtr),
         * new ConsLoRoadTripChunk( new RoadTripChunk("a", new ConsLoDirection(new
         * Direction("Switch with b", 10), this.mtr)), new ConsLoRoadTripChunk(new
         * RoadTripChunk("b", new ConsLoDirection(new
         * Direction("Make a left at Alberquerque", 3), this.mtr)), this.mtc)))
         */
        && t.checkExpect(this.test1.getTripChunk(20, 20, "a", "b", "a", this.mtr),
            new ConsLoRoadTripChunk(
                new RoadTripChunk("a", new ConsLoDirection(this.alber, this.mtr)), this.mtc));
  }
